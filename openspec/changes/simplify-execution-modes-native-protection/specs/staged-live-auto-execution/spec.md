...
The system SHALL support realtime live execution modes `manual_notify`, `small_live_auto`, and `full_live_auto`. Real automatic modes SHALL consume strategy operations through the unified execution gateway and route them through automatic execution semantics. `paper_auto` SHALL NOT be a supported realtime live execution mode.
...
**Reason**: `paper_auto` is no longer a supported realtime execution mode. Local paper account state is not part of the simplified execution design.

**Migration**: Use `manual_notify` for no-order realtime observation, or use Backtrader backtest tasks for test execution with broker-managed order semantics.

### Requirement: Paper auto supports simulated long and short positions
**Reason**: `paper_auto` is no longer a supported realtime execution mode, and local simulated long/short positions are not a target exchange abstraction.

**Migration**: Use Backtrader for strategy testing. Use real live modes only after safety gates and exchange prerequisites pass.
