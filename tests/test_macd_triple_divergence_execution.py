...
def test_signal_context_suggested_stop_price_drives_framework_stop():
    rows = [dict(open=100, high=101, low=99, close=100) for _ in range(40)] + [
        ...
        dict(open=100, high=101, low=99, close=100),
        dict(open=100, high=112, low=98, close=111.9),
    ...
    assert abs(float(first.entry_price) - 112.0) < 1e-9
...
def test_suggested_stop_price_is_atr_adjusted_when_strategy_provides_one():
...
def test_successful_next_day_macd_follow_through_keeps_trade_open():
    class _SuccessfulFollowThroughProbe(MacdTripleDivergenceStrategy):
        params = (
        ...
        @staticmethod
        def _next_day_macd_follow_through_ok(direction, entry_hist, current_hist, eps=1e-12):
            return True
    ...
    rows = [dict(open=100, high=101, low=99, close=100) for _ in range(40)] + [
        ...
        dict(open=100, high=112, low=98, close=111.9),
        dict(open=112, high=114, low=108, close=110),
        dict(open=109, high=111, low=105, close=108),
    ]

    cerebro = bt.Cerebro()
    cerebro.addstrategy(_SuccessfulFollowThroughProbe)
    data_feed = bt.feeds.PandasData(dataname=_build_df(rows), datetime="datetime")
    cerebro.adddata(data_feed)
    cerebro.broker.setcash(100000.0)
    cerebro.broker.setcommission(commission=0.0)
    ...
    trades = [t for t in st._trades_by_id.values() if t.entry_price is not None]  # noqa: SLF001
    assert len(trades) == 1
    trade = trades[0]
    assert float(trade.entry_price) == 112.0
    assert trade.exit_price is None
    assert trade.exit_reason_code is None
...
def test_strategy_private_exit_can_close_trade_without_framework_breakeven_or_tp():
    ...
    assert float(trade.exit_price) == 109.0
    assert trade.exit_reason_code == "strategy_stop"
    assert trade.exit_reason_label == "策略止损逻辑退出"


def test_strategy_stop_at_completed_entry_bar_preempts_next_bar_trailing_stop():
...
def test_strategy_immediate_exit_race_is_classified_and_not_unclassified():
    ...
    rows = [dict(open=100, high=101, low=99, close=100) for _ in range(40)] + [
        dict(open=100, high=101, low=99, close=100),
        dict(open=100, high=101, low=99, close=100),
        dict(open=100, high=110, low=98, close=109.9),
