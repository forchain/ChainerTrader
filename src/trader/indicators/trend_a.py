...
class TrendIndicatorA(bt.Indicator):
    ...
    params = dict(ma_type="EMA", ma_period=9, alma_offset=0.85, alma_sigma=6)
    plotinfo = dict(subplot=False)

    def __init__(self):
        ha = self.data  # 已是 Heikin-Ashi 数据
        self.lines.open_ma = get_ma(
            ha.open,
            self.p.ma_period,
            self.p.ma_type,
            self.p.alma_offset,
