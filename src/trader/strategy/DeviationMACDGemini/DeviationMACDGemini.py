...
class PivotHigh(bt.Indicator):
    ...
    def next(self):
        ...
        for i in range(1, self.p.period + 1):
            if pivot_price < self.p.source[-self.p.period + i] or \
               pivot_price < self.p.source[-self.p.period - i]:
                is_pivot = False
                break
        if is_pivot:
...
class DeviationMACDGeminiStrategy(BaseStrategy):
    ...
    def positive_divergence_check(self, regular=True):
        ...
        for i in range(len(self.pl_positions)):
            ...
            if len_to_pivot > self.p.div_max_bars: break
            if len_to_pivot <= 5: continue
